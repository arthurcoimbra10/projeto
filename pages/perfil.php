...
$perfilEmail = isset($_GET['usuario']) && is_string($_GET['usuario']) ? strtolower(trim($_GET['usuario'])) : $usuario['email'];
if ($perfilEmail !== strtolower($usuario['email'])) {
	redirecionar('perfil-publico.php?usuario=' . urlencode($perfilEmail));
}
$contasDemo = isset($_SESSION['contas_demo']) && is_array($_SESSION['contas_demo']) ? $_SESSION['contas_demo'] : [];
$perfil = $perfilEmail === $usuario['email'] ? $usuario : ($contasDemo[$perfilEmail] ?? null);
if (!$perfil) {
	redirecionar('perfil.php');
}
$ehMeuPerfil = $perfil['email'] === $usuario['email'];
$nome = $perfil['nome'];
$iniciais = '';
...
$portfolioErro = '';
$portfolioAviso = '';
$portfolio = $_SESSION['portfolios'][$perfil['email']] ?? [];
$avaliacaoErro = '';
$avaliacaoAviso = '';
$avaliacoes = $_SESSION['avaliacoes'][$perfil['email']] ?? [];
$fotoErro = '';
$fotoAviso = '';

function dados_avaliador_perfil($nome, $usuario, $contasDemo)
{
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && campo('acao') === 'atualizar_foto') {
	if (!formulario_valido()) {
		$fotoErro = 'Sua sessão expirou. Tente novamente.';
	} elseif (!$ehMeuPerfil || !isset($_FILES['foto_perfil'])) {
		$fotoErro = 'Não foi possível atualizar a foto.';
	} elseif ($_FILES['foto_perfil']['error'] !== UPLOAD_ERR_OK) {
		$fotoErro = 'Selecione uma imagem válida.';
	} elseif ($_FILES['foto_perfil']['size'] > 2 * 1024 * 1024) {
		$fotoErro = 'A foto deve ter no máximo 2 MB.';
	} else {
		$informacoesImagem = @getimagesize($_FILES['foto_perfil']['tmp_name']);
		$tiposPermitidos = ['image/jpeg', 'image/png', 'image/gif', 'image/webp'];
		$tipoImagem = $informacoesImagem['mime'] ?? '';
		if (!$informacoesImagem || !in_array($tipoImagem, $tiposPermitidos, true)) {
			$fotoErro = 'Envie uma imagem JPG, PNG, GIF ou WEBP.';
		} else {
			$fotoData = file_get_contents($_FILES['foto_perfil']['tmp_name']);
			$foto = 'data:' . $tipoImagem . ';base64,' . base64_encode($fotoData);
			$_SESSION['usuario']['foto'] = $foto;
			$_SESSION['contas_demo'][$usuario['email']]['foto'] = $foto;
			$usuario['foto'] = $foto;
			$perfil['foto'] = $foto;
			$fotoAviso = 'Foto de perfil atualizada.';
		}
	}
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && campo('acao') === 'adicionar_avaliacao') {
	...
	$nota = (int) campo('nota');
	if ($ehMeuPerfil) {
		$avaliacaoErro = 'Você só pode avaliar outros usuários.';
	} elseif (!formulario_valido()) {
		...
		$_SESSION['avaliacoes'][$perfil['email']] = $avaliacoes;
	...
	<main class="profile-layout">
		...
		<section class="profile-content" aria-labelledby="profile-title">
			...
			<div class="profile-card">
				<p class="profile-eyebrow"><?= $ehMeuPerfil ? 'Minha conta' : 'Perfil público' ?></p>
				<h1 id="profile-title">Perfil</h1>
				<div class="profile-avatar" aria-label="Foto de <?= escapar($nome) ?>">
					<?php if (!empty($perfil['foto'])): ?>
						<img src="<?= escapar($perfil['foto']) ?>" alt="Foto de <?= escapar($nome) ?>">
					<?php else: ?>
				...
				<?php if ($ehMeuPerfil): ?>
					<?php if ($fotoAviso): ?><p class="auth-message" role="status"><?= escapar($fotoAviso) ?></p><?php endif; ?>
					<?php if ($fotoErro): ?><p class="auth-message auth-error" role="alert"><?= escapar($fotoErro) ?></p><?php endif; ?>
					<form class="profile-photo-form" action="perfil.php" method="post" enctype="multipart/form-data">
						<input type="hidden" name="csrf" value="<?= escapar($_SESSION['csrf']) ?>">
						<input type="hidden" name="acao" value="atualizar_foto">
						<label for="foto-perfil">Foto de perfil</label>
						<input id="foto-perfil" name="foto_perfil" type="file" accept="image/jpeg,image/png,image/gif,image/webp" required>
						<button class="settings-button" type="submit">Atualizar foto</button>
					</form>
				<?php endif; ?>
			...
			<section class="portfolio-section" aria-labelledby="portfolio-title">
				...
				<?php if ($ehMeuPerfil): ?><form class="portfolio-form" action="perfil.php?usuario=<?= urlencode($perfil['email']) ?>" method="post">
					<input type="hidden" name="csrf" value="<?= escapar($_SESSION['csrf']) ?>">
			...
			<section class="reviews-section" aria-labelledby="reviews-title">
				...
				<h2 id="reviews-title"><?= $ehMeuPerfil ? 'Avaliações recebidas' : 'Avaliações' ?></h2>
				...
				<?php if (!$ehMeuPerfil): ?>
					<form class="portfolio-form" action="perfil.php?usuario=<?= urlencode($perfil['email']) ?>" method="post">
						<input type="hidden" name="csrf" value="<?= escapar($_SESSION['csrf']) ?>">
						<input type="hidden" name="acao" value="adicionar_avaliacao">
						<input type="hidden" name="avaliador" value="<?= escapar($usuario['nome']) ?>">
